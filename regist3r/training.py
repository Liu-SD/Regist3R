...
from regist3r.model import Regist3R, inf  # noqa: F401, needed when loading the model
from regist3r.losses import *  # noqa: F401, needed when loading the model
from regist3r.inference import loss_of_one_step  # noqa
from regist3r.utils import generate_random_rotation, MetricLogger
...
def get_args_parser():
    ...
    # model and criterion
    parser.add_argument('--model', default="Regist3R(patch_embed_cls='ManyAR_PatchEmbed')",
                        type=str, help="string containing the model to build")
    parser.add_argument('--pretrained', default=None, help='path of a starting checkpoint')
    parser.add_argument('--train_criterion', default="ConfLoss(Regr3DRegist3R(L21), alpha=0.2)",
    ...
    for epoch in range(args.start_epoch, args.epochs + 1):
        ...
        if epoch > args.start_epoch:
            if args.keep_freq and epoch % args.keep_freq == 0:
                ...
                model_to_delete = epoch - 2 * args.keep_freq
                if model_to_delete > 0 and misc.is_main_process():
                    fn_to_delte = os.path.join(args.output_dir, f"checkpoint-{model_to_delete}.pth")
                    if os.path.exists(fn_to_delte):
                        os.remove(fn_to_delte)
            if new_best:
        ...
        # Train
        train_stats = track_train_one_epoch(
...
def track_train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    model.train(True)
    metric_logger = MetricLogger(delimiter="  ", window_size=args.print_freq)
    ...
    for data_iter_step, batch in enumerate(metric_logger.log_every(data_loader, args.print_freq, header)):
        ...
        random_rotation = generate_random_rotation(batch[0]['img'].shape[0], device=batch[0]['pts3d'].device)
        for view in batch:
            view['pts3d'] = geotrf(random_rotation, view['pts3d'])  # B,H,W,3
            view['camera_pose'] = random_rotation @ view['camera_pose']

        total_loss_value = 0.
        total_loss_details = defaultdict(float)
        N = len(batch) - 1
        for i in range(N):
            loss_tuple = loss_of_one_step((batch[i], batch[i+1]), model, criterion, device, ret='loss')
        ...
        total_loss_value /= N
        for k in total_loss_details:
            total_loss_details[k] /= N
        lr = optimizer.param_groups[0]["lr"]
        metric_logger.update(epoch=epoch_f)
        metric_logger.update(lr=lr)
        metric_logger.update(loss=total_loss_value, **total_loss_details)

        if (data_iter_step + 1) % accum_iter == 0 and ((data_iter_step + 1) % (accum_iter * args.print_freq)) == 0:
            total_loss_value_reduce = misc.all_reduce_mean(total_loss_value)  # MUST BE EXECUTED BY ALL NODES
            ...
            log_writer.add_scalar('train/loss', total_loss_value_reduce, epoch_1000x)
            log_writer.add_scalar('train/lr', lr, epoch_1000x)
            log_writer.add_scalar('train/iter', epoch_1000x, epoch_1000x)
            for name, val in total_loss_details.items():
                log_writer.add_scalar('train/' + name, val, epoch_1000x)
        del batch
...
def test_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    for _, batch in enumerate(metric_logger.log_every(data_loader, args.print_freq, header)):
        loss_tuple = loss_of_one_step(batch, model, criterion, device, ret='loss')
    ...
    if log_writer is not None:
        for name, val in results.items():
            log_writer.add_scalar(prefix + '/' + name, val, 1000 * epoch)
